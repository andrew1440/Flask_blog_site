...
def comment(post_id):
    ...
    # if form.validate_on_submit():
    #     comment = form.comment.data
    #     new_comment = Comment( post_id=id, comment_post=comment, user_id=current_user._get_current_object().id)
    #     db.session.add(new_comment)
    #     db.session.commit()
    #     # new_comment.save_comments()
    #     return redirect(url_for('main.comments', post_id=post_id))
    ...
    # quote = get_quote()
    # post = Post.query.get(post_id)
    # comment = request.form.get("formm")
    # new_comment = Comment(comment_post=comment,user_id=current_user._get_current_object().id,post_id=post_id)
    # db.session.add(new_comment)
    # db.session.commit()
